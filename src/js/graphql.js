...
//             console.log("xpData", xpData[i].amount / 1000 + ' kb', xpData[i].path);
//         }
...
//     }
// }
// const renderXpGraph = (xpData) => {
//     if (!xpData || xpData.length === 0) {
//         console.error("No XP data available for rendering the graph.");
//         return;
//     }

//     // Calculate the maximum XP value from the data
//     const maxXP = Math.max(...xpData.map(data => data.amount));
...
//     svg.innerHTML = xpData.map((data, index) => {
//         const barHeight = (data.amount / maxXP) * svgHeight;
//         const x = barWidth * index;
//         const y = svgHeight - barHeight;

//         // Create the bar
//         const rect = `<rect width="${barWidth - barPadding}" height="${barHeight}" x="${x}" y="${y}" fill="grey"></rect>`;

//         // Position for the project name text
//         const textX = x + (barWidth / 2);
//         const textY = svgHeight + 20; // Adjust this value to move the text lower or higher

//         // Create the text element for the project name
//         const text = `<text x="${textX}" y="${textY}" font-size="12" text-anchor="middle">${data.projectName}</text>`;
...
const renderXpGraph = (xpDataForGraph) => {
    ...
    svgContent += xpDataForGraph.map((data, index) => {
        ...
        const y = barHeight * index + 30; // Offset y position to make room for the Total XP text
